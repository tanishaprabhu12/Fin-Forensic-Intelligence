...
The UI is structured into three dedicated operational tabs built with a dark cyber forensic design system:
1. **Tab 1: Alerts Feed (Triage Queue):** Real-time triage list with multi-parameter filtering (Priority, Protocol, Minimum Risk Score, Text Search), risk progress pills, and JSON/CSV batch export.
...
3. **Tab 3: Explainability & SAR Dossier Card:** Deep-dive transaction inspector with raw telemetry, CIOH entity profile, interactive SHAP waterfall/bar attribution plot, triggered compliance reason cards, and one-click Suspicious Activity Report (SAR) Markdown generation.

---

## 8. Verification & Performance Benchmarks

The automated test suite (`pytest tests/test_pipeline.py`) validates all architectural tiers:

```
tests/test_pipeline.py::test_pydantic_valid_transaction PASSED           [ 12%]
tests/test_pipeline.py::test_pydantic_invalid_ipv4 PASSED                [ 25%]
tests/test_pipeline.py::test_pydantic_negative_amount PASSED             [ 37%]
tests/test_pipeline.py::test_ingestion_pipeline_enrichment PASSED        [ 50%]
tests/test_pipeline.py::test_common_input_ownership_heuristic PASSED     [ 62%]
tests/test_pipeline.py::test_heterogeneous_graph_construction PASSED     [ 75%]
tests/test_pipeline.py::test_feature_engineering_and_model_scoring PASSED [ 87%]
...
- **Offline Guarantee:** 100% self-contained—zero external API calls, local GeoIP/ASN threat database, and cached ML model weights.
